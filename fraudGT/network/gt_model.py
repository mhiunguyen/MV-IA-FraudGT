...
class GTModel(torch.nn.Module):
    ...
    def forward(self, batch):
        # MV-IA-FraudGT keeps a direct view of the normalized transaction
        # features (including port numbering) before the edge encoder and
        # FraudGT layers transform them.  The tensor is attached to the batch
        # only immediately before the prediction head so it cannot interfere
        # with HeteroData.to_homogeneous() inside the encoder blocks.
        raw_task_edge_attr = None
        if cfg.gt.head == 'multiview_hetero_edge':
            task = cfg.dataset.task_entity
            raw_task_edge_attr = batch[task].edge_attr
        ...
        if raw_task_edge_attr is not None:
            batch[cfg.dataset.task_entity].raw_edge_attr = raw_task_edge_attr
