...
class Logger(object):
    ...
    def classification_binary(self):
        ...
        stats = {
            ...
            'precision': round(precision_score(true, pred_int,
                                               zero_division=0), cfg.round),
            'recall': round(recall_score(true, pred_int,
                                         zero_division=0), cfg.round),
            'f1': round(f1_score(true, pred_int,
                                 zero_division=0), cfg.round),
            'auc': round(r_a_score, cfg.round),
        }

        # Log the same predictions at several thresholds. The final threshold
        # must be selected on validation data, never directly on the test set.
        ...
        for threshold in thresholds:
            ...
            stats[f'precision_t{suffix}'] = round(
                precision_score(true, threshold_pred, zero_division=0),
                cfg.round)
            stats[f'recall_t{suffix}'] = round(
                recall_score(true, threshold_pred, zero_division=0),
                cfg.round)
            stats[f'f1_t{suffix}'] = round(
                f1_score(true, threshold_pred, zero_division=0), cfg.round)
        return stats
